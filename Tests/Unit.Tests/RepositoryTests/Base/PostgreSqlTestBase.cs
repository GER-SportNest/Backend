...
{
    protected PostgreSqlTestBase(PostgreSqlRepositoryTestDatabaseFixture fixture, ITestOutputHelper outputHelper, string? prefix = "T", Guid? dbId = null)
        : base(outputHelper)
    {
...
///     The type of the core database context used in tests.
/// </typeparam>
public abstract class PostgreSqlTestBase<TApplication, TApi, TDbContext> : GenericTestBase<TApplication, TApi, TDbContext>
    where TDbContext : DbBaseContext
{
    protected PostgreSqlTestBase(PostgreSqlRepositoryTestDatabaseFixture fixture, ITestOutputHelper outputHelper, string? prefix = "T", Guid? dbId = null)
        : base(outputHelper)
