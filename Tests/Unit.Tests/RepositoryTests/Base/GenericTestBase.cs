    ...
    protected virtual async Task BuildServiceProvider(bool useMigrate = true)
    {
        ...
        foreach (var contextFactory in dbContextFactories)
        {
            ...
            try
            {
                ...
                tasks.Add(useMigrate ? context.Database.MigrateAsync() : context.Database.EnsureCreatedAsync());
            }
            catch (Exception)
            {
                await context.Database.EnsureDeletedAsync();
    ...
    protected async Task ClearDatabase(IDbContextFactory contextFactory)
    {
        ...
        lock (db.Database)
        {
            if (db.Database.CanConnect())
            {
                ...
                var truncateSql = $"TRUNCATE {string.Join(", ", tableNames)} RESTART IDENTITY CASCADE;";
